heatMaps <- function(variableList,varRank=NULL,Outcome,data,title="Heat Map",hCluster=FALSE,prediction=NULL,Scale=FALSE,theFiveColors=c("blue","cyan","black","yellow","red"),outcomeColors = c("blue","lightgreen","yellow","orangered","red"),transpose=FALSE,...) 
{
  ...
  color.bar <- function(lut, min, max=-min, nticks=3, ticks=seq(min, max, len=nticks),transpose=FALSE, Title='',...) 
  {
    ...
    par(new=TRUE,cex=0.5,mai=c(0,0,0,0),...)
...
sprintf("%0.1f", ticks)))
  ...
  else
  {
    ...
    cm <- as.data.frame(cor(t(orderData[,-1]),method="spearman"))
    cm[is.na(cm)] <- -2;
    ...
    if (is.null(prediction))
    {
      ...
      else
      {
        ...
        heatMap <- gplots::heatmap.2(t(orderData), 
                                     ...
                                     col=my_palette,       	# use on color palette defined earlier 
                                     breaks=col_breaks,    	# enable color transition at specified limits
                                     dendrogram="row", 		# cluster by column
                                     ColSideColors=rowcolors,
                                     #                                     lmat=rbind(c(0,5,0,4,0), c(0,3,1,2,0)),
                                     #                                     lhei=c(1.5,4.5),
