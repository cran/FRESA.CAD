...
function(variableList,formula,Outcome,data,categorizationType=c("Raw","Categorical","ZCategorical","RawZCategorical","RawTail","RawZTail"),type=c("LOGIT","LM","COX"),rankingTest=c("zIDI","zNRI","IDI","NRI","NeRI","Ztest","AUC","CStat","Kendall"),cateGroups=c(0.1,0.9),raw.dataFrame=NULL,description=".",uniType=c("Binary","Regression"),fullAnalysis=TRUE) 
{
	...
	baseResiduals <- residualForNeRIs(bmodel,data,Outcome);
	...
	if (type=="COX")
	{
		...
		if (length(termslist)==0)
		{
			...
			baseResiduals <- residualForNeRIs(bmodel,coxframe,Outcome);
		}
	}
		...
		if (length(table(data[,colnamesList[j]]))>4)
		{
			if (uniType=="Binary")
			{
				...
				if (!is.na(cateGroups[1]))
				{
					...
					caseCount1 = eval(parse(text = paste("nrow(subset(data,",paste(subtestt,"))")))); 
					subtestt <- paste(Outcome,paste("== 0 & ",paste(colnamesList[j],paste("<",zthr))));
					controlCount1 = eval(parse(text = paste("nrow(subset(data,",paste(subtestt,"))")))); 
					...
					if (!is.na(cateGroups[categories]))
					{
						...
						subtestt <- paste(Outcome,paste("== 1 & ",paste(colnamesList[j],paste(">",zthr))));
						caseCount2 = eval(parse(text = paste("nrow(subset(data,",paste(subtestt,"))")))); 
						subtestt <- paste(Outcome,paste("== 0 & ",paste(colnamesList[j],paste(">",zthr))));
						controlCount2 = eval(parse(text = paste("nrow(subset(data,",paste(subtestt,"))")))); 
					}
					else
					{
		...
		for (n in 1:categories)
		{
			...
			else
			{
				if (fullAnalysis)
				{
					...
					varResiduals <- residualForNeRIs(lmodel,data,Outcome);
