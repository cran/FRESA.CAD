bootstrapVarElimination <- function (object,pvalue=0.01,Outcome="Class",data,startOffset=0, type = c("LOGIT", "LM","COX"),selectionType=c("zIDI","zNRI"),bootLoops=250,bootFraction=1.00) 
{
  	seltype <- match.arg(selectionType)



	boot.var.IDISelection <- function (object,pvalue=0.01,Outcome="Class",dataframe,startOffset=0, type = c("LOGIT", "LM","COX"),selectionType=c("zIDI","zNRI"),bootLoops,bootFraction) 
	{
		seltype <- match.arg(selectionType)
		type <- match.arg(type);
...
#		termList <- as.list(attr(terms(object),'term.labels'))
		...
		ftmp <- formula(frm1);
		idiCV <- bootstrapValidation(bootFraction,bootLoops,ftmp,Outcome,dataframe,type,plots=FALSE,startC=NULL)
	...
	{
		idiCV <- bootstrapValidation(1.0,bootLoops,model$formula,myOutcome,mydataFrame,type,plots=FALSE,startC=NULL);
	}
	else
	{
		idiCV <- bk$BootModel;
